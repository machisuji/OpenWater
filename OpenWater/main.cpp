...
void addPixelShader(osg::Node* node)
{
	osg::Program* program = new osg::Program;
	osg::StateSet* state = node->getOrCreateStateSet();
	osg::Shader* shader = osgDB::readShaderFile("red.frag");
...
int main( int argc, char **argv )
{
	...
	osg::PositionAttitudeTransform* posTransform = new osg::PositionAttitudeTransform();
	posTransform->addChild(loadedModel.get());

	loadedModelTransform->addChild(posTransform);

	osg::Vec3 position(5,5,0);
	posTransform->setPosition(position); 

    osg::ref_ptr<osg::NodeCallback> nc = new osg::AnimationPathCallback(
		loadedModelTransform->getBound().center(), osg::Vec3(0.0f,0.5f,1.0f), osg::inDegrees(45.0f));
